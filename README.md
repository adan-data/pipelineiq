...
A full-stack analytics project built on 18 months of SaaS customer data (598 customers, Jan 2022 to Jun 2023). The Streamlit dashboard surfaces churn drivers, cohort retention, survival economics, pricing elasticity, marketing attribution, and a live churn risk model. All findings are reproducible via SQL against `saas_intel.db`.

---

## What This Project Demonstrates

This is an end-to-end analytics workflow that moves from raw, dirty data to prioritised, quantified business decisions. The goal at every step was to answer a specific business question, not to demonstrate a technique.
...
The project answers that through six interconnected analyses, each building on the last, plus a deployed churn risk classifier that scores customers at signup time.
...
**Why it matters for decision-making:** The correct response is not "get more Starters." It is "fix months 1 to 3 for Starters, or shift acquisition budget toward Pro and Enterprise where unit economics are healthy."

### 2. Churn Is Worsening Structurally

Monthly churn rose from 5.97% (Q1 2022) to 9.37% (Q2 2023), a 57% relative increase over six quarters. This pattern holds across all tiers and cohorts. It is not driven by one bad quarter or one bad channel. Something in the product or onboarding experience has been degrading.
...
### 3. Annual Billing Shows Adverse Selection, Not Retention Benefit

The conventional wisdom in SaaS is that annual billing reduces churn because customers are financially committed for 12 months. This data contradicts that: annual billing customers churn at 8.17%/mo vs 6.85% for monthly, and the gap is consistent within every tier (Enterprise annual: 5.06% vs 2.30% monthly).
...
Enterprise represents 45% of active MRR with only 15% of customers. Monthly churn is 3.10%, breakeven is month 10, and 74% of Enterprise customers survive to that point. Gross LTV is approximately $6,000, net LTV approximately $4,200, which is 323x the expected net LTV of a Starter customer.

The gap analysis (NB08, benchmarked against OpenView 2024 and KeyBanc 2024) shows Enterprise LTV:CAC of 3.27x is the only metric at benchmark level. Everything else (NRR, logo churn, Magic Number) is critical or below benchmark.

**Why it matters:** The growth strategy the data implies is straightforward. Stop acquiring Starters at scale, invest in Starter onboarding to improve survival to breakeven, and allocate acquisition budget toward Enterprise. One additional Enterprise customer per month generates more expected LTV than 30 additional Starter customers.
...
The log-log demand model (NB06) estimates price elasticity per tier from observed conversion data. Enterprise shows elasticity of approximately -0.8, meaning inelastic demand: a 10% price increase reduces conversion by only 8%, producing net positive revenue. Enterprise customers buy on functionality and integration cost, not price sensitivity.

Pro shows elasticity of approximately -2.1, highly elastic. The model estimates the revenue-maximising Pro price is $40 to $50, well below the current $74. The wide confidence interval (±35%) means the exact number is uncertain, but the direction is robust across the full CI range: Pro is overpriced and generating lower conversion revenue than a lower price would.
...
- **Honest uncertainty quantification:** every model includes confidence intervals, explicit assumptions, and a required limitations section. The pricing notebook has a section titled "What This Is and What It Is Not."
- **Methodological justification:** Ridge over OLS because LASSO eliminates channels needed in the attribution model; customer-months churn rate instead of `churned.mean()` because the latter is a lifetime proportion, not a rate.
...
- **Deployed ML with production hygiene:** the Churn Risk Model page trains a calibrated GBM on signup-time features only, uses a temporal train/test split to prevent leakage, applies Platt scaling so scores are actual probabilities, and documents every model decision with a business rationale in a sign-off table.
...
```bash
git clone https://github.com/yourname/pipelineiq.git
cd pipelineiq
...
│   ├── 03_churn_model_development.ipynb # GBM + permutation feature importance
...
| Churn Risk Model | Calibrated GBM classifier: ROC curves, permutation importance, calibration curve, score distribution, model sign-off table |
...
| Churn model AUC | 0.73 (temporal holdout) | -- | Useful for triage |
...
| 03 | Churn Model | What features predict churn at signup time and how reliable is the model? |
...
| 5_Cohort Retention | Retention % grid: 18 cohorts x Mo 0-12 |
...
| mrr | float | Monthly recurring revenue, cleaned to $5-$500 |
...
**Churn rate definition:** All churn rates use the customer-months method: `churned / total customer-months observed`. This produces a monthly rate. The figure `churned.mean() = 68.2%` is the lifetime churn proportion, not a monthly rate, and should not be compared to the 2%/mo industry benchmark.
...
**Churn risk model:** The GBM classifier uses a temporal train/test split (80/20 by signup date) to reflect production conditions. Platt scaling is applied so predicted probabilities correspond to observed churn rates, not just rankings. The model is trained on signup-time features only (tier, billing cycle, channel, log MRR, cohort quarter) to avoid leakage from tenure or churn date. AUC on the temporal holdout is 0.73. The 0.65 decision threshold balances precision and recall for CSM triage workload.

**Pricing and MMM estimates:** Observational estimates carry approximately ±35% uncertainty. They indicate direction and order of magnitude, not precise optimal values. All pricing recommendations require A/B validation (experimental designs in NB09) before full rollout.
